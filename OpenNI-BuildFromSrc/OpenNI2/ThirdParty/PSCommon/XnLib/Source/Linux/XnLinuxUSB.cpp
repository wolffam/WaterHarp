...
XN_C_API XnStatus xnUSBOpenDeviceByPath(const XnUSBConnectionString strDevicePath, XN_USB_DEV_HANDLE* pDevHandlePtr)
{
	...
	XnUInt8 nBus = 0;
	XnUInt8 nAddress = 0;
	int scanned=sscanf(strDevicePath, "%hx/%hx@%hhu/%hhu", &nVendorID, &nProductID, &nBus, &nAddress);
	fprintf(stderr,  "nVendorID=%d, nProductID=%d, nBus=%d, nAddress=%d\n", nVendorID, nProductID, nBus, nAddress);

	/* On some systems, bus number 0 is a valid possibility; use scan count for error check instead of 0 values */
	if (scanned!=4)
	{
	...
	libusb_device** ppDevices;
	ssize_t nDeviceCount = libusb_get_device_list(g_InitData.pContext, &ppDevices);
	fprintf(stderr,"Device count = %ld\n", nDeviceCount);
	...
	fprintf(stderr,"xnUSBOpenDeviceImpl() -> %d\n",nRetVal);
	XN_IS_STATUS_OK(nRetVal);
